...
The sequence is also available at \cite{oeisA375865}, currently computed up to
$n=28$.

Plot of $S_n$ in semi-log scale is visually indistingusihable from 
a line. It's more interesting to plot $r_n=S_{n+1}/S_n$, which is shown in
Fig.~\ref{fig:rn}. This plot suggests that $S_n \sim C \cdot \mu^n$ where
$\mu \approx 3.64$. The rest of the paper is devoted to fomalizing this 
hypothesis.
...
The purpose of these boundary conditions is to allow safe
concatenation. Translate the next block so that its first cube
is the cube reached by the preceding block's exit step.
