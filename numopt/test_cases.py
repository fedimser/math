...
class OptimizationTestCase:
    def __init__(self, name="", start_point=[0], func=lambda x: x, grad=None, expected_argmin=None, gtol=1e-4):
        self.name = name
    ...
    def get_func_value(self, x):
        if x.shape[0] != self.dim:
            raise ValueError("Wrong dimension: %d!=%d" % (s.shape[0], self.dim))
    ...
    def _get_numerical_grad_value(self, x):
        ...
        return np.array([self.func(x + eps * e(i)) - f_x for i in range(self.dim)]) / eps
        ...
        if self.grad is None:
            ...
            if not np.allclose(gr_exact, gr_num, atol=1e-1, rtol=1e-1):
                raise ValueError(
                    ...
                    "x=%s, grad_exact=%s, grad_num=%s, diff=%s" % (x, gr_exact, gr_num, diff)
                )
            return gr_exact
    ...
    def check_algorithm(self, algo):
        ...
        self.prepare()
        if self.grad is None:
            print("Warning! Gradient is not defined for %s, will approximate" % self.name)
        x_found = algo(self.get_func_value, self.start_point, self.get_grad_value, gtol=self.gtol)
        correct = self.near(x_found, self.expected_argmin)
        if not correct:
            print(
                "Wrong answer. Expected %s, found %s. Grad norm=%s"
                % (str(self.expected_argmin), str(x_found), np.linalg.norm(self.get_grad_value(x_found)))
            )
        return {
            "test_case": self.name,
    ...
    def compare(self, algo1, algo2, verbose=False):
        ...
        for maxiter in range(100):
            x1 = algo1(self.get_func_value, self.start_point, self.get_grad_value, gtol=self.gtol, maxiter=maxiter)
            x2 = algo2(self.get_func_value, self.start_point, self.get_grad_value, gtol=self.gtol, maxiter=maxiter)
            dir1 = _normalize(x1 - prev_point)
            dir2 = _normalize(x2 - prev_point)
            if not self.near(x1, x2):
                print("Diverged at step %d: %s vs %s" % (maxiter, str(x1), str(x2)))
...
TEST_CASES.append(
    OptimizationTestCase(
        ...
        func=lambda x: 100 * np.sum(np.square(x[1:] - np.square(x[:-1]))) + np.sum(np.square(1 - x[:-1])),
        grad=lambda x: np.concatenate([[0], 200 * (x[1:] - np.square(x[:-1]))])
        + np.concatenate([-400 * (x[1:] - np.square(x[:-1])) * x[:-1] - 2 * (1 - x[:-1]), [0]]),
...
TEST_CASES.append(himmelblau_tc([50, -50], [-2.805118, 3.131312], "Himmelblau v2"))
TEST_CASES.append(himmelblau_tc([50, 50], [-3.779310, -3.283186], "Himmelblau v3"))
TEST_CASES.append(himmelblau_tc([-50, 50], [3.584428, -1.848126], "Himmelblau v4"))
...
TEST_CASES.append(
    OptimizationTestCase(
        name="McCormick",
        start_point=[0, 0],
        func=lambda x: np.sin(x[0] + x[1]) + np.square(x[0] - x[1]) - 1.5 * x[0] + 2.5 * x[1] + 1,
        grad=lambda x: np.array(
            [
